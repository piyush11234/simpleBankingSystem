...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($acc_no) || empty($amount)) {
        ...
        // Verify the account belongs to the user and has sufficient balance
        $account = null;
        ...
        if (!$account) {
            ...
            if ($stmt->execute([$amount, $acc_no])) {
                ...
                $stmt = $pdo->prepare("INSERT INTO transactions (acc_no, type, amount) VALUES (?, 'withdraw', ?)");
                $stmt->execute([$acc_no, $amount]);
                
                $success = "Successfully withdrew $" . number_format($amount, 2) . " from account $acc_no";
            } else {
        ...
        <div class="md:flex">
            <div class="w-full p-4">
                <div class="text-center mb-6">
                    <h1 class="text-2xl font-bold text-gray-800">Withdraw Money</h1>
                </div>
